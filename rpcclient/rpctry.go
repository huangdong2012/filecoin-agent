...
func (rc *rpcClient) tryCall(method string, args interface{}, reply interface{}) error {
	...
	for i := 0; i < 3; i++ {
		call := <-client.Go(method, args, reply, make(chan *rpc.Call, 1)).Done
		err = call.Error

		return err

		// TODO:  do retry for the net error
	}
	return err
}
...
func (rc *rpcClient) Call(method string, args interface{}, reply interface{}) error {
	if err := rc.tryCall(method, args, reply); err != nil {
		if _, ok := err.(rpc.ServerError); ok {
			rc.disconn()
			time.Sleep(ReConnSleepTime)
			return errors.As(rc.tryCall(method, args, reply))
		}
